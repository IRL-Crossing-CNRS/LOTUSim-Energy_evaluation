...
positions_D = [1, 9, 18, 27]
positions_m = [d * D for d in positions_D]
 
CFD_DIR = os.path.join(DATA_DIR, 'layout_b', 'wakeProfiles')
...
model_profiles = {n: {} for n in ['LOTUSim-Jensen', 'LOTUSim-Gaussian', 'LOTUSim-Larsen']}
for pos_m in positions_m:
    label = f'{int(round(pos_m / D))}D'
    print(f"  {label}...")
...
styles = {
    'CFD':          dict(color='black',          lw=2.5, ls='-',  marker='o', ms=8),
    'FLORIS Gauss': dict(color='royalblue',      lw=2.0, ls='--', marker='s', ms=7),
    'LOTUSim-Jensen':   dict(color='tomato',         lw=1.5, ls='--', marker='v', ms=7),
    'LOTUSim-Gaussian': dict(color='#DAA000',        lw=1.5, ls='--', marker='D', ms=7),
    'LOTUSim-Larsen':   dict(color='mediumseagreen', lw=1.5, ls='--', marker='P', ms=7),
}
...
for idx, (pos_m, ax) in enumerate(zip(positions_m, axes)):
    ...
    ax.axvline(-0.5, color='steelblue', lw=1.0, ls=':', alpha=0.45)
    ax.axvline( 0.5, color='steelblue', lw=1.0, ls=':', alpha=0.45,
    ...
    ax.legend(fontsize=11, loc='lower center', ncol=3,
...
_out = os.path.join(SUPP_DIR, 'wake_profiles_4x4.png')
fig1.savefig(_out, dpi=600, bbox_inches='tight')
print("\nFigure 1 saved:", _out)
...
for ax, transform, ylabel, ylim, title in [
    ...
    for tp, tn in [(0,'T1'),(7,'T2'),(14,'T3')]:
        ax.axvline(tp, color='steelblue', lw=1.0, ls=':', alpha=0.5)
...
os.makedirs(SUPP_DIR, exist_ok=True)
_out = os.path.join(SUPP_DIR, 'wake_shape_4x4_centreline_decay.png')
fig2.savefig(_out, dpi=600, bbox_inches='tight')
...
fig3.tight_layout()
os.makedirs(SUPP_DIR, exist_ok=True)
_out = os.path.join(SUPP_DIR, 'wake_shape_4x4_profile_rmse.png')
fig3.savefig(_out, dpi=600, bbox_inches='tight')
